...
for (semilla in 1:k) {
  ...
  png(paste("p4","d",n,"s",k,"s.png",sep="_"))
  par(mar = c(0,0,0,0))
  image(rotate(zona), col=rainbow(k+1), xaxt='n', yaxt='n')
  graphics.off()
  png(paste("p4","d",n,"s",k,"c.png",sep="_"))
  par(mar = c(0,0,0,0))
  ...
  propaga <- function(replica) {
    ...
    if (largo >= limite) {
      png(paste("p4","d",n,"s",k,replica,".png",sep="_"))
      par(mar = c(0,0,0,0))
  ...
  suppressMessages(library(doParallel))
  registerDoParallel(makeCluster(detectCores() - 1))
  largos <- foreach(r = 1:ciclos, .combine=c) %dopar% propaga(r)
  stopImplicitCluster()
